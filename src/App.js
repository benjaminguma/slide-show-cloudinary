...
export default function App() {
	...
	const onCapture = (newImage) => setImages((prevImages) => [...prevImages, newImage]);

	const deleteImage = (ind) => setImages((prevImages) => images.filter((_, index) => index !== ind));
		...
		<main>
			...
			<section className='captured_imags_con con'>
				{images.map((imgURL, index) => (
					<ImagePreviewer url={imgURL} key={index} del={() => deleteImage(index)} />
				))}
			</section>
		</main>
	);
